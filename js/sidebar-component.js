...
(function() {
  ...
  window.initSidebar = async function() {
    ...
    const inPoster  = path.includes('/App/Poster/');
    const inTasker  = path.includes('/App/Tasker/');
    ...
    const posterRoot  = inPoster  ? '' : '../Poster/';
    const taskerRoot  = inTasker  ? '' : '../Tasker/';
    ...
    if (sidebarEl) {
      ...
      sidebarEl.innerHTML = `
        ...
        <nav class="sidebar-nav">
          ...
          <a href="${settingsRoot}account.html" class="sidebar-link ${activeTab === 'settings' ? 'is-active' : ''}" data-tab="settings">
