/**
 * Taska Tasker Profile Controller (Tasker Module)
...
window.renderStandaloneProfile = async function (targetProfileId) {
  ...
  // Check if username is in the URL pathname (e.g. /tasker/profile/johndoe)
  if (!identifier) {
  ...
  if (profileToRender.username && window.history && window.history.replaceState) {
    ...
    if (window.location.search !== cleanSearch) {
      window.history.replaceState(null, '', `/tasker/profile${cleanSearch}`);
    }
  }
