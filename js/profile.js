...
window.renderStandaloneProfile = async function (targetProfileId) {
  ...
  if (taglineEl) {
    const title = profileToRender.taskerTitle || profileToRender.posterName;
    if (title) {
      taglineEl.textContent = title;
  ...
  const rawSkills = profileToRender.taskerSkills || profileToRender.posterCategories || profileToRender.skills || '';
  ...
  if (aboutBioText) aboutBioText.textContent = profileToRender.bio || profileToRender.taskerBio || profileToRender.posterBio || 'This user has not added a bio yet.';
  ...
  // 4. Load Dynamic Task History & Real Stats
...
async function loadProfileTaskHistory(profileId) {
  ...
  try {
    ...
    const postedTasks = tasks ? tasks.filter((t) => t.posterId === profileId) : [];
    const activeTasks = tasks ? tasks.filter((t) => t.status === 'IN_PROGRESS' || t.status === 'ASSIGNED') : [];
    ...
    if (statTasksPosted) statTasksPosted.textContent = postedTasks.length;
    if (statActiveHires) statActiveHires.textContent = activeTasks.length;
    if (aboutDetailTasks) aboutDetailTasks.textContent = completedTasks.length;
    if (aboutDetailTasksPosted) aboutDetailTasksPosted.textContent = postedTasks.length;
